...
$ambilMAX2  = mysql_query("SELECT MAX(Tinggi_Badan) as max, MIN(Tinggi_Badan) as min FROM data_uji order by id DESC LIMIT 1");
$dataMAX2   = mysql_fetch_array($ambilMAX2);
$ips1max    = $dataMAX2 ['max'];
$ips1min    = $dataMAX2 ['min'];

$ambilMAX3  = mysql_query("SELECT MAX(Berat_Badan) as max, MIN(Berat_Badan) as min FROM data_uji order by id DESC LIMIT 1");
$dataMAX3   = mysql_fetch_array($ambilMAX3);
$ips2max    = $dataMAX3 ['max'];
$ips2min    = $dataMAX3 ['min'];

$ambilMAX4  = mysql_query("SELECT MAX(Lingkar_Kepala) as max, MIN(Lingkar_Kepala) as min FROM data_uji order by id DESC LIMIT 1");
$dataMAX4   = mysql_fetch_array($ambilMAX4);
$ips3max    = $dataMAX4 ['max'];
$ips3min    = $dataMAX4 ['min'];

$ambilMAX5  = mysql_query("SELECT MAX(Lingkar_Lengan) as max, MIN(Lingkar_Lengan) as min FROM data_uji order by id DESC LIMIT 1");
$dataMAX5   = mysql_fetch_array($ambilMAX5);
$ips4max    = $dataMAX5 ['max'];
$ips4min    = $dataMAX5 ['min'];

$ambilMAX6  = mysql_query("SELECT MAX(Umur) as max, MIN(Umur) as min FROM data_uji order by id DESC LIMIT 1");
...
$ambil_n = mysql_query("SELECT * FROM data_uji WHERE kelas_klasifikasi = ''  ORDER BY id DESC LIMIT 1");
...
echo "<script type='text/javascript'>
   window.location.replace('index2.php?page=hasil-klasifikasi');
  </script>";
